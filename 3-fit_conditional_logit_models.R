...
tapply(modDat$abs_SPL_2000dB_std.OG, modDat$case, function(x) c(mean(x), sd(x))) 
...
# PLACEHOLDER -  FEMALE WIND + SPL MODEL ----------------------------------
...
# PLACEHOLDER - FEMALE - SPL*WIND DIR PREDICTIONS -------------------------
...
# 2.5.2 Get scaling attributes for SPL
att_SPL <- attributes(scale(modDat.M$abs_SPL_2000dB_std.OG, center = TRUE, scale = TRUE))
mylabels_SPL <- seq(50,75,5)
mybreaks_SPL <- scale(mylabels_SPL, att_SPL$`scaled:center`, att_SPL$`scaled:scale`)[,1]
